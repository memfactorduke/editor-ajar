...
- **Encode/export (`AjarExport`, EXP):** exact sequential frame pulls execute the render graph;
  ...
  publication, and the same exact-time, original-only graph contract; still/GIF delivery uses its
  codec-free image initializer. Output is converted and tagged per ADR-0010, then atomically
  published (FR-EXP-005/006/007, ADR-0019).
