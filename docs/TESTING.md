...
is sampled every iteration. The run fails with a typed error and the growth curve when any
post-warmup sample rises more than the growth band above the baseline-window median (default
64 MiB — roughly 10x the observed benign malloc/driver-pool jitter, far below what a real
per-iteration leak accumulates), or when the four post-warmup quartile means increase strictly
monotonically by more than 8 MiB.

Cadence: CI runs `ajar soak --duration-seconds 150` on every PR (~3 minutes). The **NFR-STAB-005
acceptance run** is the full 1-hour soak — `ajar soak --duration-seconds 3600` — executed before
releases (nightly wiring may adopt it later).
