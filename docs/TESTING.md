...
| **Release acceptance** | app-model end-to-end usable-app journey (create → import → edit → compound make/open/edit/return → save/reopen → ProRes export decode → decompose/undo), plus production app-path animated-GIF submission and ImageIO decode; H.264 capability-skipped on encoder-less runners (#236/#269/#279) | every PR (EditorAjarTests / ui-smoke) | seconds–min |
