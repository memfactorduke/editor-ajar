    ...
    var body: some View {
        ...
        RoundedRectangle(cornerRadius: 3)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 3)
                    .stroke(
                        isSelected ? Color.accentColor : Color.white.opacity(0.62),
            ...
            .rotationEffect(.degrees(display.rotationDegrees))
            .position(
                x: display.center.x + dragPreview.width,
                y: display.center.y + dragPreview.height
            )
            ...
            .accessibilityLabel("Title text box \(layout.boxIndex + 1), \(layout.clipName)")
            .accessibilityValue(accessibilityValue)
            .accessibilityHint(
                layout.isEditable
                    ? "Press Return to edit. Drag or use arrow keys to move."
                    : "This title is on a locked track."
            )
...
private struct CanvasSafeAreaGuides: View {
    ...
    private func guideRectangle(
        ...
        return ZStack(alignment: .topLeading) {
            Rectangle()
                .stroke(color.opacity(0.9), style: StrokeStyle(lineWidth: 1, dash: [6, 4]))
            Text(label)
