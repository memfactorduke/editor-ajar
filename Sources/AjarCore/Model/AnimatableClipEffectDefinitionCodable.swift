...
extension AnimatableClipEffectDefinition {
    /// Decodes a kind-tagged keyframable payload, defaulting a missing payload by kind.
    ...
    private static func decodeBatch2(
        ...
        case .vignette:
            return .vignette(
                try container.decodeIfPresent(
                    AnimatableClipVignetteSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        case .mirror:
            return .mirror(
                try container.decodeIfPresent(
                    AnimatableClipMirrorSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        case .mosaic:
            return .mosaic(
                try container.decodeIfPresent(
                    AnimatableClipMosaicSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        case .colorAdjust:
            return .colorAdjust(
                try container.decodeIfPresent(
                    AnimatableClipColorAdjustSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        case .posterize:
            return .posterize(
                try container.decodeIfPresent(
                    AnimatableClipPosterizeSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        case .invert:
            return .invert(
                try container.decodeIfPresent(
                    AnimatableClipInvertSettings.self,
                    forKey: .parameters
                ) ?? .identity
            )
        default:
    ...
    /// Encodes kind + typed keyframable parameters.
